...
import { NationalityDto, OccupationDto, CasteDto, SocietyDto, SubCasteDto, MotherTongueDto, ClassDto, ReligionDto, BranchDto, SchoolDto, SchoolTypeDto } from 'app/views/administration/administration';
...
@Component({
  ...
  styleUrls: ['./student-admission-form.component.scss']
})
export class StudentAdmissionFormComponent implements OnInit {
 ...
 // country=[{name:'India',code:42},{name:'Nepal',code:85},{name:'Bangladesh',code:69}];
  ...
  countryData:any;
  stateData:any;
  StudentAddressDetails:FormGroup;
  ...
  this.releventForm=this.fb.group({
    identityByMark:[''],
    className:[''],
    religionName:[''],
    casteName:[''],
...
.subscribe(res=>{
  ...
  getDropDownlistData(){
    ...
    this.adminService.getAllCountry().subscribe(res=>{console.log(res); this.countryData=res.data});  
      ...
      getadmissionDetails(index:number){
        console.log('stepper.selectedIndex: ' + this.selectedIndex 
        + '; goto index: ' + index);
        if (index == 0) return; // First step is not selected anymore -ok
    this.selectedIndex = index;
      ...
      getreleventDetails(index:number){
        console.log('stepper.selectedIndex: ' + this.selectedIndex 
        + '; goto index: ' + index);
        if (index == 0) return; // First step is not selected anymore -ok
        this.selectedIndex = index;

      this.admissionDetails.identityByMarkOrAadharNo=this.releventForm.get('identityByMark').value;
      this.admissionDetails.classId=this.releventForm.get('className').value;
      this.admissionDetails.religionId=this.releventForm.get('religionName').value;
      this.admissionDetails.castId=this.releventForm.get('casteName').value;
      this.admissionDetails.subCastId=this.releventForm.get('subCaste').value;
      this.admissionDetails.occupationId=this.releventForm.get('occupation').value;
      this.admissionDetails.societyId=this.releventForm.get('society').value;
      this.admissionDetails.nationalityId=this.releventForm.get('nationality').value;
      this.admissionDetails.mothertongId=this.releventForm.get('mothertongue').value;
      this.admissionDetails.schoolId=this.releventForm.get('schoolId').value;
      this.admissionDetails.schoolTypeId=this.releventForm.get('schoolTypeId').value; 
      console.log(this.admissionDetails)
      }
  
      submitDocs(index:number){
        console.log('stepper.selectedIndex: ' + this.selectedIndex 
        + '; goto index: ' + index);
        if (index == 0) return; // First step is not selected anymore -ok
        this.selectedIndex = index;
